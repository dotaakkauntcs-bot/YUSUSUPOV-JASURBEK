document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    cardsContainer.style.transform = `translate(${x * 10}px, ${
      y * 10
    }px) rotateX(${y * 3}deg) rotateY(${-x * 3}deg)`;
  });
  ...
  function createMagneticParticles(container, color) {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      particle.style.animation = `pulsate ${
        1 + Math.random() * 2
  ...
  function handleReflectiveTexture(e, card) {
    ...
    if (highlight) {
      ...
      highlight.style.transform = `translate(-50%, -50%) scale(${
        1 + Math.abs(angleX * angleY)
      })`;
    }
  }
  ...
  function handleCardMouseMove(e, card) {
    ...
    if (edgeGlow) {
      ...
      edgeGlow.style.opacity =
        0.5 + intensity * 0.5 + Math.max(0, edgeProximityFactor) * 0.3;
      edgeGlow.style.filter = `blur(${
        3 + intensity * 3
  ...
  function resetCardEffects(card) {
    ...
    if (edgeGlow) {
      ...
      edgeGlow.style.background =
        "linear-gradient(135deg, var(--glow, #7b00ff), transparent 70%) border-box";
    }
    ...
    card.style.boxShadow =
      "0 15px 35px rgba(0, 0, 0, 0.5), 0 0 10px var(--glow, rgba(123, 0, 255, 0.3))";
  ...
  function activateAutoPulseEffect() {
    ...
    setInterval(() => {
      cards.forEach((card) => {
        ...
        const magneticParticles = cardFront.querySelectorAll(
          ".magnetic-particle"
        );
        magneticParticles.forEach((particle) => {
          particle.style.opacity = "0";
  ...
  function createParticle(container) {
    ...
    if (!document.querySelector("style#particle-animation")) {
      ...
      style.textContent = `
        ...
        40 + Math.random() * 40
      }px, -${80 + Math.random() * 80}px) rotate(${Math.random() * 360}deg);
                        opacity: 0;
                    }
                }
            `;
  ...
  function updateMagneticParticles(card, mouseX, mouseY, intensity) {
    ...
    particles.forEach((particle, index) => {
      ...
      const pullStrength =
        Math.max(0, 1 - Math.min(distance / maxPullDistance, 1)) *
        intensity *
        20;
      ...
      particle.style.transform = `translate(${
        normDirX * pullStrength * 0.5
      }px, ${normDirY * pullStrength * 0.5}px)`;

      particle.style.opacity = 0.3 + (pullStrength / 20) * 0.7;
      particle.style.filter = `blur(${
        1 + (pullStrength / 20) * 2
      }px) brightness(${1 + (pullStrength / 20) * 0.5})`;
    });
  }
  ...
  function addClickEffect(button, card) {
    ...
    ripple.style.animation =
      "button-ripple 0.6s cubic-bezier(0.1, 0.7, 0.3, 1) forwards";
    ...
    pulseBorder.style.boxShadow = `0 0 20px 10px ${
      card.getAttribute("data-color") || "#7b00ff"
    }`;
